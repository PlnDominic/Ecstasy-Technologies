...
import { pickTodaysImagePost, findPostById } from '@/data/social-posts';
...
// Bibiani" Facebook Page, using the shared content queue (see
// src/data/social-posts.ts).
//
// Instagram has no text-only post type — every post needs an image. So
// unlike Facebook/X, this route doesn't use the shared day-of-year pick
// directly (that includes text-only entries); it uses
// pickTodaysImagePost(), which runs the same deterministic rotation but
// only over the image-bearing subset of the queue. That means Instagram
// always has something to post, even on a day the main rotation would
// land on a text-only entry — at the cost of Instagram and Facebook
// sometimes posting different content on the same day, which is fine.
//
...
export async function GET(request: Request) {
  ...
  const post = overrideId ? findPostById(overrideId) : pickTodaysImagePost();
  if (!post) {
    return NextResponse.json(
      { success: false, message: overrideId ? `No post with id "${overrideId}".` : 'No image posts in the queue.' },
      { status: overrideId ? 404 : 200 }
    );
  }

  if (!post.image) {
    // Only reachable via ?postId= pointing at a text-only entry —
    // pickTodaysImagePost() itself never returns one.
